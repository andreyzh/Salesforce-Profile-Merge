            ...
            foreach (var element in targetDoc.Root.Elements())
            {
                ...
                foreach (var kvp in componentDefinitions)
                {
                    ...
                    if (localName == kvp.Key)
                    {
                        ...
                        // SearchTerm is the unqiue name of the component
                        var target =
                            from el in sourceDoc.Root.Elements(nameSpace + localName)
                            where (string)el.Element(nameSpace + kvp.Value) == searchTerm
                            select el;
...
Source:
<layoutAssignments>
  <layout>Account-ZNMC Account Layout</layout>
  <recordType>Account.KONE_companies_head_office</recordType>
</layoutAssignments>

Target:
<layoutAssignments>
  <layout>Account-ZNMC Account Layout</layout>
  <recordType>Account.ZNMC</recordType>
</layoutAssignments>
*/